...
class DecisionTree:
    ...
    def entropy(y, sample_weights):
        ...
        if sample_weights is not None:
            P0=np.sum(sample_weights[y==0])/np.sum(sample_weights)
            P1=np.sum(sample_weights[y==1])/np.sum(sample_weights)
        else:
    ...
    def gini_impurity(y, sample_weights):
        ...
        if sample_weights is not None:
            for v in values:
                gini-=(np.sum(sample_weights[y==v])/np.sum(sample_weights))**2
        else:
            for v in values:
